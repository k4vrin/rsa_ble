package dev.kavrin.rsable.data.util
...
object ScanRecordParser {
    ...
    private fun getAdvertisementData(advertisementDataBuffer: ByteArray): String? {
        val flagsLength = advertisementDataBuffer[0]
        val advertisementData: StringBuilder
        if (flagsLength.toInt() == 0) {
            return null
        }
    ...
    private fun getManufacturerSpecificData(data: ByteArray): String {
        val builder = StringBuilder()
        if (data.size >= 2) {
            builder.append("Company Code: 0x").append(Converters.getHexValue(data[1])).append(
                Converters.getHexValue(data[0])
            ).append("").append("<br/>")
            builder.append("Data: ")
            //Hessom >
            var hm_DataStr: String = ""
            //Hessom <
            if (data.size > 2) {
                builder.append("0x")
                for (i in 2..<data.size) {
                    ...
                    //Hessom >
                    val charValue = parseLong(Converters.getHexValue(data[i]), 16).toInt().toChar()
                    hm_DataStr += charValue
                    //Hessom <
                }
                //Hessom >
                if (hm_DataStr.length >= 15) {
                    Log.d(" ####################### Name > ", hm_DataStr.substring(14))
                    builder.append("<br/>")
                    builder.append("Wesko Name: ")
                    builder.append(hm_DataStr.substring(14))
                }
                //Hessom <
